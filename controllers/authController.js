...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!token) {
    return next(
      new AppError('You are not currently logged in. Login to continue', 401)
    );
  }
  // 2) verify the token
...
exports.isLoggedIn = async (req, res, next) => {
  if (req.cookies.jwt) {
    try {
      ...
      // 3) check if the user changed password after the JWT was issued
      if (currentUser.changedPasswordAfter(decoded.iat)) {
        return next();
      }
...
exports.restrictTo = (...roles) => {
  return (req, res, next) => {
    // roles is an incoming array ie ['admin', 'lead-guide']
    if (!roles.includes(req.user.role)) {
      return next(
        new AppError('you do not have the proper role for this action', 403)
      );
    }
    next();
  };
